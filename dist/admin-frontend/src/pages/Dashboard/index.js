...
import { Card, Col, Row, Statistic, Table, Tag, Progress, Avatar, List, Typography, Space, Button } from 'antd';
import { UserOutlined, MessageOutlined, TeamOutlined, RiseOutlined, ArrowUpOutlined, EyeOutlined, LikeOutlined, CommentOutlined } from '@ant-design/icons';
...
const Dashboard = () => {
    ...
    return (<div style={{ padding: '24px', background: '#f0f2f5', minHeight: '100vh' }}>
      <div style={{ marginBottom: '24px' }}>
        <Title level={2}>仪表盘</Title>
        <Text type="secondary">欢迎回来！这里是您的管理后台概览</Text>
      </div>
      ...
      <Row gutter={[16, 16]} style={{ marginBottom: '24px' }}>
        <Col xs={24} sm={12} lg={6}>
          <Card>
            <Statistic title="总用户数" value={11280} precision={0} valueStyle={{ color: '#3f8600' }} prefix={<UserOutlined />} suffix={<ArrowUpOutlined style={{ fontSize: '12px' }}/>}/>
        ...
        <Col xs={24} sm={12} lg={6}>
          <Card>
            <Statistic title="今日消息" value={1563} precision={0} valueStyle={{ color: '#1890ff' }} prefix={<MessageOutlined />} suffix={<ArrowUpOutlined style={{ fontSize: '12px' }}/>}/>
            <div style={{ marginTop: '8px' }}>
        ...
        <Col xs={24} sm={12} lg={6}>
          <Card>
            <Statistic title="在线用户" value={892} precision={0} valueStyle={{ color: '#52c41a' }} prefix={<TeamOutlined />}/>
            <div style={{ marginTop: '8px' }}>
        ...
        <Col xs={24} sm={12} lg={6}>
          <Card>
            <Statistic title="系统性能" value={98.5} precision={1} valueStyle={{ color: '#722ed1' }} prefix={<RiseOutlined />} suffix="%"/>
            <div style={{ marginTop: '8px' }}>
      ...
      <Row gutter={[16, 16]}>
        ...
        <Col xs={24} lg={8}>
          ...
          <Card title="最近用户" style={{ marginBottom: '16px' }}>
            <List itemLayout="horizontal" dataSource={recentUsers} renderItem={(item) => (<List.Item>
                  <List.Item.Meta avatar={<Avatar style={{
                    backgroundColor: item.status === 'online' ? '#52c41a' :
                        item.status === 'away' ? '#faad14' : '#d9d9d9'
                }} icon={<UserOutlined />}/>} title={item.name} description={item.time}/>
                  <Tag color={item.status === 'online' ? 'green' :
          ...
          <Card title="最近消息">
            <List itemLayout="vertical" size="small" dataSource={recentMessages} renderItem={(item) => (<List.Item actions={[
                <Space key="actions">
                  ...
                  <List.Item.Meta title={<Text strong>{item.user}</Text>} description={item.time}/>
