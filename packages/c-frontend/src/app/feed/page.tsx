...
import { getPosts, createPost } from "@/lib/posts.api"
...
import { ApiError, NetworkError, Comment } from "@/types/common"
...
export default function FeedPage() {
  ...
  const [comments, setComments] = useState<Record<string, { total: number, items: Comment[] }>>({})
  ...
  // 从当前加载的帖子中提取活跃用户（按发帖数量排序，取前 6 名）
  ...
  function renderMarkdown(md: string) {
    ...
    let html = "\n" + md
    // 代码块 ``` ```
    html = html.replace(/```([\s\S]*?)```/g, (_: string, code: string) => `<pre class="rounded border bg-muted/40 p-3 overflow-auto"><code>${escapeHtml(code.trim())}</code></pre>`)
    // 行内代码 `code`
    html = html.replace(/`([^`]+?)`/g, (_: string, code: string) => `<code class="px-1 py-0.5 rounded bg-muted/60">${escapeHtml(code)}</code>`)
    // 标题 #
  ...
  async function toggleLike(p: { id: string; likedByMe?: boolean; likeCount?: number }) {
    try {
      if (p.likedByMe) {
    ...
    <main className="min-h-dvh p-6 max-w-7xl mx-auto flex flex-col gap-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              {!isLoggedIn ? (
                ...
                <>
                  {preview ? (
                    <div className="border border-border rounded p-3 text-sm min-h-[96px] bg-muted/20" dangerouslySetInnerHTML={{ __html: renderMarkdown(draft || '') }} />
                  ) : (
                    <Textarea
                      ref={editorRef}
                      placeholder="分享此刻的想法..."
                        ...
                        <input
                          ...
                          onChange={async (e) => {
                            ...
                            try {
                              ...
                              const error = err as ApiError | NetworkError | Error
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="min-w-0 flex-1">
                              ...
                              {m.images.map((u, i) => {
                                ...
                                return (
                                  <img
                                    ...
                                    src={url}
                                    alt=""
                                    className="w-full h-auto object-contain rounded border border-border bg-muted cursor-zoom-in"
                          ...
                          <div className="flex items-center gap-4 mt-2 text-xs">
                            ...
                            {((user?.id && String(user.id) === m.authorId) || (Array.isArray(user?.roles) && user.roles.some((r: string | { name: string }) => (typeof r === 'string' ? r : r?.name) === 'admin'))) ? (
                                  ...
                                  <div className="flex flex-col gap-1">
                                    ...
                                    <div className="flex items-center gap-2">
                                      <Input
                                        ...
                                        className="flex-1 h-8 text-xs rounded-full px-3 border border-border"
      ...
      {confirmDeleteId ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-background rounded-md shadow-lg w-[90%] max-w-sm p-4 border border-border">
