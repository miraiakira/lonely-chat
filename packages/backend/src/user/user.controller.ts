...
// Strongly-typed helpers to avoid `any` unsafe access
...
export class UserController {
  ...
  async findRecentActive(@Query('limit') limit?: string) {
    ...
    const ids: string[] = await this.redis.zrevrange(
      'recent:active_users',
      0,
      take - 1,
    );
    ...
    const users = await Promise.all(
      ids.map((id: string) => this.userService.findOne(Number(id))),
    );
  ...
  update(
    @Param('id') id: string,
    @Body() body: { user: UpdateUserDto; profile: UpdateUserProfileDto },
  ) {
    return this.userService.update(+id, body.user, body.profile);
  }

  // 新增：当前登录用户更新自己的个人资料（兼容 PUT/PATCH）
  @Patch('profile')
  @UseGuards(JwtAuthGuard)
  updateMyProfilePatch(
    @Request() req: ReqWithUser,
    @Body() profile: UpdateUserProfileDto,
  ) {
    return this.userService.update(+req.user.id, {} as UpdateUserDto, profile);
  }
