...
export class SeedService {
  ...
  async seed() {
    ...
    if (adminUser) {
      this.logger.log(
        'Existing superadmin found. Reconciling roles/permissions and profile...',
      );
      // 不再强制覆盖头像，仅对齐昵称与角色
      ...
      await this.userService.assignRoles(adminUser.id, {
    ...
    for (const du of demoUsers) {
      ...
      try {
        ...
        this.logger.warn(
          `[seed] create demo user failed (${du.username}): ${e instanceof Error ? e.message : e}`,
        );
      }
    }
